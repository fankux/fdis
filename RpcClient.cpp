...
void RpcClient::CallMethod(const google::protobuf::MethodDescriptor* method,
    ...
    pthread_cond_t* provider_cond = &mutex_cond_pair->second;
    ...
    pthread_cond_wait(provider_cond, provider_lock);
    InvokePackage* pop_package = invoke_queue->pop();
    if (pop_package == NULL || pop_package != package) {
        warn("stranger occured, invoking queue package addr:%d while package addr:%d",
                (int) pop_package, (int) package);
    }
    delete package;
    pthread_mutex_unlock(provider_lock);
}
...
Queue<InvokePackage>* RpcClient::get_invoke_queue(const int sid, const int mid,
    ...
    if (invoke_queue == NULL) {
        invoke_queue = new(std::nothrow) Queue<InvokePackage>(1, false);
...
bool RpcClient::populate_invoke_package(InvokePackage* package, const int sid, const int mid,
    ...
    if (package->_package_len >= sizeof(package->_send_buffer)) {
        // TODO send for some batch
        error("too large package, len: %d, max len: %zu", request_len,
                sizeof(package->_send_buffer));
        return false;
    }

//    debug("meta len: %d, request_len: %d, ModelRequest len: %d", meta_len, request_len,
//            ((ModelRequest*) request)->ByteSize());
    memcpy(package->_send_buffer, &package->_package_len, 4);
    memcpy(package->_send_buffer + 4, &meta_len, 4);
    bool ret = meta.SerializeToArray(package->_send_buffer + 8, meta_len);
    if (!ret) {
        error("serialize meta faild");
        return false;
    }
    ret = request->SerializeToArray(package->_send_buffer + 8 + meta_len, request_len);
    if (!ret) {
...
void RpcClient::connect(struct event* ev) {
    sockaddr_in addr;
    addr.sin_family = AF_INET;
    addr.sin_port = htons(_port);
    addr.sin_addr.s_addr = inet_addr(_addr.c_str());
    event_connect(_netinf, ev, &addr);
}
...
int RpcClient::invoke_callback(struct event* ev) {
    ...
    ssize_t write_len = write_tcp(ev->fd, package->_send_buffer, (size_t) package->_package_len);
    if (write_len == -1) {
...
int RpcClient::return_callback(struct event* ev) {
    ...
    bool ret = package->_response->ParseFromArray(buffer + 8 + status_len,
...
//    }
    ...
    info("call succ, response, msg: %d", &response);
    request.Clear();
    response.Clear();
...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < num; ++i) {
        pthread_create(&pid, NULL, call_routine, model_service);
    }
